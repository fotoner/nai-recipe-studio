...
export function createRecipeStudioMcpServer(client: Pick<RpcClient, "request">) {
  ...
  const tools: Array<{ name: string; command: string; description: string; readOnly: boolean; destructive?: boolean; idempotent?: boolean; inputSchema?: z.ZodType }> = [
    ...
    { name: "generation_prepare", command: "generation.prepare", description: "Prepare a fixed generation plan and estimate; this does not generate images.", readOnly: false, inputSchema: z.object({ recipe: Recipe, count: z.number().int().min(1).max(200), seed: z.number().int().min(0).max(4294967295).optional() }) },
    { name: "generation_start", command: "generation.start", description: "Start an already approved generation plan.", readOnly: false, destructive: true, inputSchema: z.object({ planId: z.string().min(1).max(200), requestId: z.string().min(1).max(200) }) },
