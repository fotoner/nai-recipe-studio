...
import { LIBRARY_SORTS, sortLibraryItems } from "@/lib/library-sort";
import { cx } from "@/features/shared/ui";
import { newRecipe, type RecipeSummary } from "@/features/shared/types";
...
import { useStoredChoice } from "@/features/shared/useStoredChoice";
import { StyleBlock } from "@/lib/schema";
...
type RecipesFeatureProps = { client?: StudioClient; onOpenRecipe: (id: number) => void };
type RecipeView = "cards" | "table";
const RECIPE_VIEWS = ["cards", "table"] as const;
...
export function RecipesFeature({ client, onOpenRecipe }: RecipesFeatureProps) {
  ...
  const [view, setView] = useStoredChoice<RecipeView>("studio.recipes.view", "cards", RECIPE_VIEWS);
  const [sort, setSort] = useStoredChoice("studio.recipes.sort", "usage", LIBRARY_SORTS);
  ...
  const create = async () => {
    ...
    setBusy(true);
    try {
      const recipe = newRecipe(`${t("recipes.newRecipe")} ${new Date().toISOString().slice(5, 16).replace("T", " ")}`);
      try {
        const savedStyle = JSON.parse(localStorage.getItem("studio.default_style") ?? "null");
        const style = StyleBlock.safeParse(savedStyle?.block);
        if (style.success) recipe.blocks = recipe.blocks.map(block => block.type === "style" ? style.data : block);
      } catch { /* Invalid or unavailable local preferences keep the neutral default. */ }
      const saved = await studioCall(client, "recipes.save", { recipe });
      onOpenRecipe(saved.id);
    }
  ...
  const translatedError = error && error.startsWith("errors.") ? t(error) : error;
...
function LintBadge({ row, inline = false }: { row?: RecipeLintRow; inline?: boolean }) {
  ...
  return <span className={cx("flex gap-1", !inline && "absolute right-1.5 bottom-1.5")}>{errors ? <Badge variant="destructive" className="h-4 gap-0.5 px-1 text-[10px]"><CircleAlert className="size-3" />{errors}</Badge> : null}{warnings ? <Badge variant="outline" className="h-4 gap-0.5 bg-background px-1 text-[10px] text-amber-500"><TriangleAlert className="size-3" />{warnings}</Badge> : null}{!errors && !warnings ? <Badge variant="secondary" className="h-4 gap-0.5 px-1 text-[10px]"><CircleCheck className="size-3 text-emerald-500" />{t("common.ready")}</Badge> : null}</span>;
}

function RecipeCard({ recipe, lint, blurSensitive, onOpen, onDuplicate, onDelete }: { recipe: RecipeSummary; lint?: RecipeLintRow; blurSensitive: boolean; onOpen: () => void; onDuplicate: () => void; onDelete: () => void }) {
  const { t } = useTranslation();
  const [reveal, setReveal] = React.useState(false);
  const latest = recipe.latest;
  const hidden = !!latest && blurSensitive && latest.rating >= 1 && !reveal;
  const date = (latest?.created_at ?? recipe.updated_at ?? "").replace("T", " ").slice(5, 16);
  return <div className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-card transition-colors hover:border-foreground/30">
    <div className="relative aspect-3/4 w-full overflow-hidden bg-muted">
      ...
      <LintBadge row={lint} />
      {(recipe.generation_count ?? 0) > 1 ? <Badge variant="secondary" className="pointer-events-none absolute bottom-1.5 left-1.5 h-4 px-1 font-mono text-[10px]">{t("recipesExtra.imageCount", { count: recipe.generation_count })}</Badge> : null}
    ...
    <div className="flex flex-col gap-1.5 p-2.5"><button type="button" className="truncate text-left text-sm font-medium hover:underline" onClick={onOpen} title={recipe.name}>{recipe.name || t("common.untitled")}</button><div className="flex flex-wrap items-center gap-1">{recipe.tags.slice(0, 2).map((tag) => <Badge key={tag} variant="secondary" className="h-4 px-1 text-[10px]">{tag}</Badge>)}{recipe.tags.length > 2 ? <span className="text-[10px] text-muted-foreground">+{recipe.tags.length - 2}</span> : null}</div><div className="flex items-center justify-between font-mono text-[10px] text-muted-foreground"><span title={latest?.created_at ?? recipe.updated_at}>{date}</span><span className="flex opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100"><Button size="icon-xs" variant="ghost" aria-label={t("common.duplicate")} onClick={onDuplicate}><Copy /></Button><Button size="icon-xs" variant="ghost" aria-label={t("common.delete")} onClick={onDelete}><Trash2 /></Button></span></div></div>
  </div>;
}

function RecipeTable({ recipes, blurSensitive, lintBy, onOpen, onDuplicate, onDelete }: { recipes: RecipeSummary[]; blurSensitive: boolean; lintBy?: Map<number, RecipeLintRow>; onOpen: (id: number) => void; onDuplicate: (recipe: RecipeSummary) => void; onDelete: (recipe: RecipeSummary) => void }) {
  const { t } = useTranslation();
  return <div className="overflow-x-auto rounded-xl border border-border"><Table>
    <TableHeader><TableRow>
      <TableHead className="w-12" aria-label={t("recipesExtra.thumbnail")} />
      <TableHead>{t("recipesExtra.name")}</TableHead>
      <TableHead className="hidden md:table-cell">{t("recipesExtra.tags")}</TableHead>
      <TableHead>{t("recipesExtra.rating")}</TableHead>
      {lintBy ? <TableHead>{t("editor.check")}</TableHead> : null}
      <TableHead className="hidden lg:table-cell">{t("recipesExtra.latestGeneration")}</TableHead>
      <TableHead className="text-right">{t("common.edit")}</TableHead>
    </TableRow></TableHeader>
    <TableBody>{recipes.map(recipe => <TableRow key={recipe.id}>
      <TableCell className="p-1">{recipe.latest ? <Thumbnail sizes="40px" src={recipe.latest.url} alt="" className={cx("size-10 rounded-md object-cover", blurSensitive && recipe.latest.rating >= 1 && "blur-sm")} /> : <div className="flex size-10 items-center justify-center rounded-md bg-muted text-muted-foreground"><ImageOff className="size-4" /></div>}</TableCell>
      <TableCell className="max-w-[18rem] truncate"><button type="button" className="font-medium hover:underline" onClick={() => onOpen(recipe.id)}>{recipe.name || t("common.untitled")}</button><span className="ml-2 text-[11px] text-muted-foreground">{t("recipesExtra.blockCount", { count: recipe.blocks.length })}</span></TableCell>
      <TableCell className="hidden md:table-cell"><div className="flex flex-wrap gap-1">{recipe.tags.slice(0, 3).map(tag => <Badge key={tag} variant="secondary" className="text-[10px]">{tag}</Badge>)}</div></TableCell>
      <TableCell><Badge variant={recipe.rating >= 1 ? "destructive" : "outline"}>{recipe.rating}</Badge></TableCell>
      {lintBy ? <TableCell><LintBadge row={lintBy.get(recipe.id)} inline /></TableCell> : null}
      <TableCell className="hidden font-mono text-[11px] text-muted-foreground lg:table-cell">{recipe.latest ? `${recipe.latest.created_at.replace("T", " ").slice(0, 16)} · ${t("recipesExtra.imageCount", { count: recipe.generation_count ?? 0 })}` : t("common.none")}</TableCell>
      <TableCell className="text-right whitespace-nowrap"><Button size="icon-xs" variant="ghost" aria-label={t("common.duplicate")} onClick={() => onDuplicate(recipe)}><Copy /></Button><Button size="icon-xs" variant="ghost" aria-label={t("common.delete")} onClick={() => onDelete(recipe)}><Trash2 /></Button></TableCell>
    </TableRow>)}</TableBody>
  </Table></div>;
}
