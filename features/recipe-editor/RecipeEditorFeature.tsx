...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Ban, Brush, Check, ChevronDown, Code2, Copy, Crop, Download, History, Image as ImageIcon, Layers, Loader2, MessageCircle, MoreHorizontal, Plus, Replace, RotateCcw, Save, ShieldAlert, Shirt, ShieldCheck, Smile, SlidersHorizontal, Sparkle, Sun, Trash2, WandSparkles, Users } from "lucide-react";
...
type RecipeEditorProps = { client?: StudioClient; recipeId: number; onBack: () => void; onGenerate: (recipe: Recipe) => void; onDirtyChange?: (dirty: boolean) => void };
...
export function RecipeEditorFeature({ client, recipeId, onBack, onGenerate, onDirtyChange }: RecipeEditorProps) {
  ...
  const load = React.useCallback(async () => {
    if (!client) { setRecipe(null); setLoading(false); return; }
    setLoading(true); setError(null);
    try {
      const [loaded, characterPage, presetPage] = await Promise.all([
        studioCall(client, "recipes.get", { id: recipeId }),
        studioCall(client, "characters.list", { limit: 200, offset: 0 }),
        studioCall(client, "presets.list", { limit: 200, offset: 0, includeHidden: false }),
      ]);
      const next = withCastBlock(asRecipe(loaded));
      setRecipe(next); setSaved(JSON.stringify(next)); setCharacters(asCharacters(characterPage.items)); setPresets(asPresets(presetPage.items));
    } catch (cause) { setError(errorMessage(cause, "errors.load")); }
    finally { setLoading(false); }
  }, [client, recipeId]);
  React.useEffect(() => { void load(); }, [load]);

  const dirty = !!recipe && JSON.stringify(recipe) !== saved;
  React.useEffect(() => {
    onDirtyChange?.(dirty);
    return () => onDirtyChange?.(false);
  }, [dirty, onDirtyChange]);
  const update = (next: Recipe) => { onDirtyChange?.(true); setRecipe(next); setComposed(null); };
  const save = async () => {
    ...
    try {
      ...
      setRecipe(next); setSaved(JSON.stringify(next));
      ...
      void Promise.allSettled([
        ...
        if (validateResult.status === "fulfilled") {
          ...
          if (validateResult.value.applied?.length && validateResult.value.recipe && JSON.stringify(validateResult.value.recipe) !== JSON.stringify(withoutEmptyCast(recipe))) {
            onDirtyChange?.(true);
            setRecipe(withCastBlock(asRecipe(validateResult.value.recipe)));
          }
        }
      });
    }, 300);
    return () => { cancelled = true; window.clearTimeout(timer); };
  }, [client, onDirtyChange, recipe]);
  ...
  return <div className="min-h-svh">
    <section aria-label={t("editor.basicInfo")} className="flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-border px-4 py-3 text-xs text-muted-foreground md:px-6">
      ...
      <div className="ml-auto flex items-center gap-2"><Button size="sm" variant="ghost" aria-expanded={detailsOpen} onClick={() => setDetailsOpen((value) => !value)}><Code2 />{t("editor.promptLint")}</Button><DropdownMenu><DropdownMenuTrigger render={<Button size="icon-sm" variant="ghost" aria-label={t("editor.title")} />}><MoreHorizontal /></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => setMetadataOpen((value) => !value)}><Code2 />{t("editor.notes")}</DropdownMenuItem>{recipe.id ? <DropdownMenuItem onClick={() => { if (versions) setVersions(null); else void showVersions(); }}><RotateCcw />{t("recipes.versions")}</DropdownMenuItem> : null}<DropdownMenuItem onClick={() => void validate()} disabled={busy}><ShieldCheck />{busy ? t("editor.checking") : t("editor.check")}</DropdownMenuItem><DropdownMenuItem onClick={() => void exportRecipe()}><Download />{t("common.export")}</DropdownMenuItem></DropdownMenuContent></DropdownMenu><Button size="sm" variant="outline" disabled={busy || !dirty} onClick={() => void save()}>{busy ? <RotateCcw className="animate-spin" /> : <Save />}{busy ? t("common.saving") : t("common.save")}</Button><Button size="sm" onClick={() => onGenerate(withoutEmptyCast(recipe))}><WandSparkles />{t("generation.start")}</Button></div>
    </section>
    ...
    <div className="grid grid-cols-1 items-start gap-4 p-4 md:grid-cols-[11rem_minmax(0,1fr)] md:p-6">
      <BlockPalette present={recipe.blocks.map((block) => block.type)} onAdd={appendBlock} />
      <main className="flex min-w-0 flex-col gap-4">
        {error ? <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive">{error.startsWith("errors.") ? t(error) : t("errors.unknown")}</div> : null}
    ...
    {recipe.id ? <GenerationHistory client={client} recipeId={recipe.id} /> : null}
  ...
  return <Card className="gap-0 py-0">
    <div className="flex flex-wrap items-center gap-1 px-2 py-1.5">
      ...
      <div className="ml-auto flex shrink-0 items-center gap-1">
        {block.type !== "cast" ? <Button size="xs" variant="outline" onClick={onPickPreset}><Replace />{t("editor.replace")}</Button> : null}
        {block.type !== "cast" ? <Button size="xs" variant="ghost" aria-label={`${t(BLOCK_LABEL_KEY[block.type])} ${t("editor.saveToPalette")}`} title={t("editor.savePresetTitle")} onClick={onSavePreset}><Save />{t("editor.saveBlock")}</Button> : null}
...
function GenerationHistory({ client, recipeId }: { client?: StudioClient; recipeId: number }) {
  const { t } = useTranslation();
  ...
  return <section className="flex flex-col gap-2 border-t border-border px-4 py-3 md:px-6">
    ...
    {items ? <Lightbox items={items} index={open} blur={blur} onClose={() => setOpen(null)} onIndex={setOpen} onDelete={remove} onExport={exportItem} onRate={rate} onOpenRecipe={(id) => { window.location.hash = `#/recipe/${id}`; }} /> : null}
  </section>;
}
