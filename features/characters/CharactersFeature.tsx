...
import type { GalleryItem as StoredGalleryItem, StudioClient, StoredCharacter } from "@/contracts/studio";
...
import { CharacterCard, CharacterForm, CharacterTable, emptyCharacter, type CharacterCardData, type CharacterExample, type CharacterView } from "./CharacterComponents";
...
function withGenerationExamples(characters: StoredCharacter[], items: Array<Pick<StoredGalleryItem, "id" | "recipe_id" | "seed" | "created_at" | "rating" | "url"> & { recipe?: { blocks?: unknown[] } }>): CharacterCardData[] {
  const byId = new Map<number, { count: number; examples: CharacterExample[] }>();
  for (const item of items) {
    const matched = new Set<number>();
    for (const block of item.recipe?.blocks ?? []) {
      if (typeof block !== "object" || block === null || !("members" in block) || !Array.isArray((block as { members?: unknown }).members)) continue;
      for (const member of (block as { members: unknown[] }).members) if (typeof member === "object" && member !== null && typeof (member as { character_id?: unknown }).character_id === "number") matched.add((member as { character_id: number }).character_id);
    }
    for (const id of matched) {
      const current = byId.get(id) ?? { count: 0, examples: [] };
      current.count += 1;
  ...
  return characters.map((character) => ({ ...character, examples: byId.get(character.id)?.examples ?? [], generation_count: byId.get(character.id)?.count ?? 0 }));
}
...
export function CharactersFeature({ client }: CharactersProps) {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const galleryResult = await Promise.allSettled([studioCall(client, "gallery.list", { limit: 200, offset: 0, sort: "newest" })]);
      const galleryItems = galleryResult[0].status === "fulfilled" ? galleryResult[0].value.items : [];
      setCharacters(withGenerationExamples(characters, galleryItems));
    } catch (cause) {
