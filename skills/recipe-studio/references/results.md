...
To continue from an image in the app, use the viewer's same-seed or new-seed action. It opens a separate draft from the image's recipe and actual generation settings; it does not overwrite the current recipe or generate images automatically. Older images may lack character snapshots, so their draft uses current registered character data and asks the user to reselect deleted characters. Do not promise exact reproduction for those older results.
